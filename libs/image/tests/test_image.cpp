...
static image::LinearImage createGrayFromAscii(const string& pattern);

// Creates a tiny RGB image from a pattern string.
static image::LinearImage createColorFromAscii(const string& pattern);
...
TEST_F(ImageTest, ColorFilters) { // NOLINT
    ...
    image::LinearImage color1 = createColorFromAscii("12");
    ...
    auto color4 = resampleImage(color1, 100, 100, image::Filter::LANCZOS);
    auto color5 = diffImages(color3, color4);

    // Try enlarging a 5x5 image using MITCHELL and LANCZOS filters.
    image::LinearImage color6 = createColorFromAscii("44444 41014 40704 41014 44444");
...
static image::LinearImage createColorFromAscii(const string& pattern) {
    ...
    // Allocate the sequence of pixels.
    image::LinearImage result(width, height, 3);
    ...
    for (int i = 0; istream >> row;) {
        for (char c : row) {
            ...
            seq[i++] = (c >> 1) & 1;
            seq[i++] = (c >> 2) & 1;
            float3* col = (float3*) (seq + i - 3);
            *col = image::sRGBToLinear(*col);
        }
    }
    return result;
}
...
static void updateOrCompare(const image::LinearImage& limg, const utils::Path& fname) {
    ...
    if (g_comparisonMode == ComparisonMode::UPDATE) {
        ...
        ImageEncoder::encode(out, format, img, "", fname);
        return;
    }
